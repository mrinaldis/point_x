...
const TRANSLATIONS = {
  en: {
    ...
    choose_avatar: 'Choose your Persona',
...
const App: React.FC = () => {
  const [state, setState] = useState<AppState>(() => {
    ...
    const savedLang = (localStorage.getItem('pontox_lang') as Language) || 'pt';
    
    return {
      currentUser: savedUser || { id: '1', name: 'Você', avatar: PRESET_AVATARS[0], isNearby: false, status: 'active' },
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans pb-32">
        ...
        {activeTab === 'circles' && (
          <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center px-2">
              ...
              <button onClick={() => setEditingCircle({ id: 'c' + Date.now(), name: '', icon: 'fa-users', color: 'indigo', members: [], activeMeetingPoint: null })} className="bg-indigo-600 text-white px-4 py-2 rounded-full text-[10px] font-black uppercase border border-indigo-400 shadow-lg shadow-indigo-600/20">
                + {t.new_group}
              </button>
            </div>
            ...
            <div className="space-y-4">
              {state.circles.map(circle => (
                <div key={circle.id} className={`group relative w-full flex items-center gap-5 p-6 rounded-[32px] border-2 transition-all ${state.activeCircleId === circle.id ? 'bg-indigo-600/10 border-indigo-500/50 shadow-xl shadow-indigo-600/5 scale-[1.02]' : 'bg-slate-900 border-slate-800/40 opacity-80'}`}>
          ...
          <div className="h-full flex flex-col items-center animate-in zoom-in-95 duration-500 pt-4">
            {!activeCircle.activeMeetingPoint ? (
              <div className="py-20 text-center space-y-8">
                <div className="w-24 h-24 bg-slate-900 rounded-[40px] border-2 border-slate-800 flex items-center justify-center text-indigo-500 mx-auto shadow-2xl relative">
                  <div className="absolute inset-0 bg-indigo-500/10 blur-2xl rounded-full"></div>
                  <i className="fas fa-tower-broadcast text-4xl relative"></i>
                </div>
                <h3 className="text-2xl font-black">{t.radar_off}</h3>
                <button onClick={() => handleQuickMark()} className="w-full px-12 py-5 bg-indigo-600 text-white rounded-3xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all border-t border-indigo-400">
                  {t.mark_local_now}
                </button>
              </div>
            ) : (
          ...
          <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
             <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  <input type="text" value={communityName} onChange={(e) => setCommunityName(e.target.value)} placeholder={t.community_search} className="flex-1 bg-slate-900 border-2 border-slate-800 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 transition-all" />
                  <button onClick={handleFetchEvents} disabled={loadingEvents} className="bg-indigo-600 p-4 rounded-2xl disabled:opacity-50 shadow-lg shadow-indigo-600/20">
             ...
             <div className="space-y-4">
               {state.subsplashEvents.length === 0 ? (
                 ...
                 state.subsplashEvents.map(event => (
                   <div key={event.id} className="bg-slate-900 border-2 border-slate-800 p-6 rounded-[32px] space-y-4 shadow-xl">
                     <div>
                     ...
                     <button onClick={() => handleQuickMark({ ...event, radius: 1.0 })} className="w-full py-3 bg-indigo-600/10 text-indigo-400 border-2 border-indigo-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all">
                       {t.mark_from_event}
                     </button>
                   </div>
                 ))
          ...
          <div className="space-y-10 animate-in slide-in-from-top-4 duration-500 pb-10">
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-6">
                  {PRESET_AVATARS.map(avatar => (
                    <button 
                      key={avatar} 
                      onClick={() => setState(s => ({ ...s, currentUser: { ...s.currentUser, avatar } }))} 
                      className={`relative aspect-square rounded-3xl overflow-hidden border-4 transition-all ${state.currentUser.avatar === avatar ? 'border-indigo-500 scale-110 shadow-xl shadow-indigo-600/40 bg-indigo-900/20' : 'border-slate-800 grayscale-50 bg-slate-900'}`}
                    >
                      <img src={avatar} className="w-full h-full object-cover p-1" alt="" />
                      {state.currentUser.avatar === avatar && (
                        <div className="absolute inset-0 bg-indigo-500/10 flex items-center justify-center">
                          <div className="bg-indigo-500 w-8 h-8 rounded-full flex items-center justify-center shadow-lg"><i className="fas fa-check text-white"></i></div>
                        </div>
                      )}
                    </button>
                  ))}
             ...
             <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setState(s => ({ ...s, language: 'en' }))} className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase border-2 transition-all ${state.language === 'en' ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>English</button>
                  <button onClick={() => setState(s => ({ ...s, language: 'pt' }))} className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase border-2 transition-all ${state.language === 'pt' ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg shadow-indigo-600/20' : 'bg-slate-900 border-slate-800 text-slate-500'}`}>Português</button>
                </div>
             </div>

             <div className="space-y-4 pt-6">
                <button onClick={handleReload} className="w-full py-4 bg-slate-900 border-2 border-slate-800 rounded-2xl text-xs font-black uppercase text-indigo-400 shadow-md">
                   <i className="fas fa-sync-alt mr-2"></i> {t.reload}
                </button>
                <button onClick={handleClearData} className="w-full py-4 bg-rose-600/5 border-2 border-rose-500/20 rounded-2xl text-xs font-black uppercase text-rose-500 shadow-md">
                   <i className="fas fa-trash-alt mr-2"></i> {t.clear_data}
                </button>
             </div>
          </div>
        )}
      ...
      <nav className="fixed bottom-6 left-6 right-6 z-[100] max-w-md mx-auto">
        <div className="bg-slate-900/90 backdrop-blur-2xl border border-white/10 p-2.5 rounded-[36px] flex justify-around shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex flex-col items-center px-6 py-3 rounded-3xl transition-all duration-300 ${activeTab === tab.id ? 'text-white bg-indigo-600 shadow-xl shadow-indigo-600/30 -translate-y-1' : 'text-slate-500 hover:text-slate-300'}`}>
        ...
        <div className="fixed inset-0 z-[200] bg-slate-950/90 backdrop-blur-xl p-8 flex items-center justify-center animate-in fade-in duration-300">
          <div className="w-full max-w-sm bg-slate-900 rounded-[48px] border-2 border-slate-800 p-8 space-y-8 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-6 gap-2">
                {ICONS.map(i => (
                  <button key={i} onClick={() => setEditingCircle({ ...editingCircle, icon: i })} className={`w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-sm transition-all ${editingCircle.icon === i ? 'text-white bg-indigo-600 scale-110 shadow-lg shadow-indigo-600/20' : 'text-slate-500'}`}>
                    <i className={`fas ${i}`}></i>
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-3 pt-4">
              <button onClick={saveEditedCircle} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl border-t border-indigo-400">{t.save}</button>
              {editingCircle.id && state.circles.length > 1 && (
                <button onClick={() => deleteCircle(editingCircle.id)} className="w-full py-5 bg-rose-600/10 text-rose-500 rounded-2xl font-black uppercase text-xs tracking-widest border-2 border-rose-500/20">{t.delete}</button>
              )}
